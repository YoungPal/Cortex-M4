...
void Segment ( int N )  // Segment 함수 선언
{
  ...
  int Buff ;
  int Number;
  unsigned char minute0, minute1, second0, second1;
/*
...
*/
  ...
  for( i = 0 ; i < 30; i++ )
  {
    ...
    Delay(1); 
*/
    GPIO_Write(GPIOC, Font[minute0]|0x0e00); // 왼쪽 첫 번째 세그먼트를 ON하고, 천의 자리  숫자를 출력해 준다 
    Delay(1);

    GPIO_Write(GPIOC, Font[minute1]|0x0d00); // 왼쪽 두 번째 세그먼트를 ON하고, 백의 자리  숫자를 출력해 준다 
    Delay(1); 

    GPIO_Write(GPIOC, Font[second0]|0x0b00);  // 왼쪽 세 번째 세그먼트를 ON하고, 십의 자리  숫자를 출력해 준다 
    Delay(1);

    GPIO_Write(GPIOC, Font[second1]|0x0700);   // 왼쪽 네 번째 세그먼트를 ON하고, 일의 자리  숫자를 출력해 준다 
    Delay(1); 
  }
}
